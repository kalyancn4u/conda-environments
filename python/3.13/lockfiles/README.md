...
<sub>📍 [conda-environments](../../../README.md) › [Python 3.13](../README.md) › **lockfiles**</sub>
...
> these docs are. Generate the locks on demand (see the
...
conda create --name py313-core --file python/3.13/lockfiles/linux-64/01-core.conda.lock
...
conda-lock install --name py313-tools python/3.13/lockfiles/linux-64/05-tools.conda.lock

# PROD — pinned PyPI install with uv (Python 3.13 / linux target):
uv pip install -r python/3.13/lockfiles/requirements/04-web.txt
```
...
' | tee python/3.13/lockfiles/linux-64-conda-lock.log
```
...
cd /mnt/c/Users/<you>/.../conda-environments/python/3.13
for e in 01-core 02-ml 03-deep-learning 04-web 05-tools 06-tensorflow 07-geospatial 08-timeseries; do
  conda-lock lock --file environments/$e.yml --platform linux-64 --kind explicit \
    && mv -f conda-linux-64.lock lockfiles/linux-64/$e.conda.lock
done
