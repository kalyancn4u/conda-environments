...
> [readiness note](../README.md). The uv `requirements/*.txt` are **committed for 12 of 14**
> (PyPI/linux/py3.14 — all but `06-tensorflow`/`all-in-one-tflow`, no TF wheel on PyPI);
> `win-64`/`osx-arm64` conda locks remain on demand. This page explains the artifacts and
> how to produce them.
...
conda create --name py314-core --file python/3.14/lockfiles/linux-64/01-core.conda.lock
...
conda-lock install --name py314-tools python/3.14/lockfiles/linux-64/05-tools.conda.lock

# PROD — pinned PyPI install with uv (Python 3.14 / linux target):
uv pip install -r python/3.14/lockfiles/requirements/04-web.txt
```
...
' | tee python/3.14/lockfiles/linux-64-conda-lock.log
```
...
cd /mnt/c/Users/<you>/.../conda-environments/python/3.14
for e in 01-core 02-ml 03-deep-learning 04-web 05-tools 06-tensorflow 07-geospatial 08-timeseries; do
  conda-lock lock --file environments/$e.yml --platform linux-64 --kind explicit \
    && mv -f conda-linux-64.lock lockfiles/linux-64/$e.conda.lock
done
