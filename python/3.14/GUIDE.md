# The Complete Beginner's Guide to `python/3.14/`
...
| **Environment** | One isolated, self-contained set of Python + packages with a name (e.g. `py314-core`). | One work-station with exactly the tools that job needs. |
...
> live in **one shared `scripts/` folder at the repository root** and take a `-p 3.14`
...
```yaml
name: py314-core        # the environment's name (what you 'conda activate')
...
- `dependencies:` — the packages. `python=3.14.*` means "any Python 3.14.x". Most other
...
|------|-------------|---------------|-----------------------------|
| **`01-core.yml`** | `py314-core` | The daily driver: Jupyter, NumPy, Pandas, Polars, SciPy, Matplotlib/Seaborn/Plotly, scikit-learn, DuckDB, requests/httpx, etc. Covers ~80–90% of everyday work. | Explore data, run notebooks, make charts, do light modeling. **Start here.** |
| **`02-ml.yml`** | `py314-ml` | Classical machine learning: XGBoost, LightGBM, CatBoost, Optuna (tuning), MLflow (tracking), SHAP (explanations), feature-engineering tools. | Train tabular ML models and track experiments. |
| **`03-deep-learning.yml`** | `py314-dl` | Neural networks the **PyTorch** way: PyTorch, Lightning, the Hugging Face stack (Transformers, Datasets, Accelerate), spaCy, OpenCV, ONNX. CPU builds. | Build/fine-tune neural networks, work with transformer models. |
| **`04-web.yml`** | `py314-web` | Web development: FastAPI, Flask, Django, Streamlit, Gradio, Dash, plus servers (Uvicorn/Gunicorn) and helpers (Pydantic, Alembic, Celery, Redis client). | Build an API, a dashboard, or a web app. |
| **`05-tools.yml`** | `py314-tools` | Developer tooling: pytest, coverage, ruff (linter), black (formatter), mypy (type checks), pre-commit, tox/nox, Selenium/Playwright (browser automation), Scrapy. | Test, lint, format, or automate a browser — a "toolbox" env. |
| **`06-tensorflow.yml`** | `py314-tf` | **TensorFlow + Keras**, deliberately kept in its own environment (TensorFlow and PyTorch pin conflicting low-level libraries, so mixing them breaks). | Work specifically with TensorFlow/Keras. ⚠️ **Windows note below.** |
| **`07-geospatial.yml`** | `py314-geo` | **Maps & geographic data**: GeoPandas, Shapely, PyProj, Fiona (vector), GDAL + rasterio (raster), plus folium/contextily for mapping. | Work with shapefiles, coordinates, maps, or satellite/raster data. |
| **`08-timeseries.yml`** | `py314-ts` | **Forecasting**: Prophet, sktime, statsforecast, statsmodels, plus ruptures for change-point detection. | Forecast future values, or detect shifts/seasonality in time-ordered data. |
...
|------|-------------|----------------|
| **`minimal.yml`** | `py314-min` | Want the smallest possible clean slate (just Python + a couple of niceties) and will add packages yourself. |
| **`data-science.yml`** | `py314-ds` | Prefer **one** environment that combines the core stack *and* the most-used ML tools, instead of juggling `01-core` + `02-ml`. |
| **`mlops.yml`** | `py314-mlops` | Need workflow orchestration & experiment ops: Apache Airflow, MLflow, Weights & Biases, DVC, cloud SDKs. (Heavy; Airflow is Linux/macOS-oriented.) |
| **`llm.yml`** | `py314-llm` | Are building with large language models: Transformers, Sentence-Transformers, embeddings/vector search, plus a small API server. |
| **`all-in-one-pytorch.yml`** | `py314-all-pytorch` | Want *one* env with (almost) everything and **PyTorch** as the DL framework — core + ML + PyTorch + web + tools + geospatial + time series. **Heavy & conflict-prone;** best as a scratch env, lock it once it works. |
| **`all-in-one-tflow.yml`** | `py314-all-tflow` | Same kitchen sink but with **TensorFlow/Keras** instead of PyTorch (the two can't coexist). Heavy & conflict-prone; no conda-forge win-64 TF build (pip on Windows). |
...
tree), so every command takes **`-p 3.14`** to say which Python tree to act on. Below,
"run it like this" shows the Linux/macOS (`.sh`) form; on Windows use the matching
`.ps1` with `.\` in front (e.g. `.\scripts\create-env.ps1 -p 3.14 01-core`).
...
> `-p 3.14` flag — not your current folder — is what selects the tree.
...
```bash
./scripts/create-env.sh -p 3.14 01-core          # builds the py314-core environment
```
...
```bash
./scripts/export-env.sh py314-core
```
...
```bash
./scripts/compare-envs.sh py314-core py314-ds     # list packages that differ between two envs
./scripts/compare-envs.sh --outdated py314-core   # list packages that COULD be upgraded
```
...
environment and reports ✅/❌ for each, plus a note if your Python isn't 3.14. It does
**not** test that the packages *work fully* — just that they load without errors, which
catches most broken installs.

```bash
conda activate py314-core
python scripts/verify-env.py -p 3.14 --env core     # check the core set
python scripts/verify-env.py -p 3.14 --all          # check every known set
python scripts/verify-env.py -p 3.14 --packages numpy pandas   # check your own list
```
Example of what a healthy run looks like:
```text
OK  python 3.14.x (as expected)
------------------------------------------------------------
[core]
OK  numpy      2.x
OK  pandas     2.x
...
```bash
./scripts/test-env.sh -p 3.14 01-core      # one environment  (Windows: .\scripts\test-env.ps1 -p 3.14 01-core)
./scripts/test-env.sh -p 3.14 --all        # all of them
```
...
two people creating `py314-core` a month apart might get slightly different versions. Most
...
|----------|-----------|----------------|-----------------|
| **A conda environment** (the actual installed Python + packages) | `create-env` / `conda env create` | Inside conda's own folder on your disk (e.g. `…/miniconda3/envs/py314-core`), **not** in this repo | No — it's on your machine only |
...
# 2) Build the daily-driver environment. (Linux/macOS shown; Windows: .\scripts\create-env.ps1 -p 3.14 01-core)
...
entirely later: `conda env remove -n py314-core`.

That's it — you now understand every file in `python/3.14/`, what it does, and what each
action produces.
