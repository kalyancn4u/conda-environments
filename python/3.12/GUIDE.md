# The Complete Beginner's Guide to `python/3.12/`
...
| **Environment** | One isolated, self-contained set of Python + packages with a name (e.g. `py312-core`). | One work-station with exactly the tools that job needs. |
...
> live in **one shared `scripts/` folder at the repository root** and take a `-p 3.12`
...
```yaml
name: py312-core        # the environment's name (what you 'conda activate')
...
- `dependencies:` — the packages. `python=3.12.*` means "any Python 3.12.x". Most other
...
|------|-------------|---------------|-----------------------------|
| **`01-core.yml`** | `py312-core` | The daily driver: Jupyter, NumPy, Pandas, Polars, SciPy, Matplotlib/Seaborn/Plotly, scikit-learn, DuckDB, requests/httpx, etc. Covers ~80–90% of everyday work. | Explore data, run notebooks, make charts, do light modeling. **Start here.** |
| **`02-ml.yml`** | `py312-ml` | Classical machine learning: XGBoost, LightGBM, CatBoost, Optuna (tuning), MLflow (tracking), SHAP (explanations), feature-engineering tools. | Train tabular ML models and track experiments. |
| **`03-deep-learning.yml`** | `py312-dl` | Neural networks the **PyTorch** way: PyTorch, Lightning, the Hugging Face stack (Transformers, Datasets, Accelerate), spaCy, OpenCV, ONNX. CPU builds. | Build/fine-tune neural networks, work with transformer models. |
...
|------|-------------|----------------|
| **`minimal.yml`** | `py312-min` | Want the smallest possible clean slate (just Python + a couple of niceties) and will add packages yourself. |
...
tree), so every command takes **`-p 3.12`** to say which Python tree to act on. Below,
"run it like this" shows the Linux/macOS (`.sh`) form; on Windows use the matching
`.ps1` with `.\` in front (e.g. `.\scripts\create-env.ps1 -p 3.12 01-core`).
...
> `-p 3.12` flag — not your current folder — is what selects the tree.
...
```bash
./scripts/create-env.sh -p 3.12 01-core          # builds the py312-core environment
```
...
```bash
./scripts/compare-envs.sh py312-core py312-ds     # list packages that differ between two envs
./scripts/compare-envs.sh --outdated py312-core   # list packages that COULD be upgraded
```
...
environment and reports ✅/❌ for each, plus a note if your Python isn't 3.12. It does
...
python scripts/verify-env.py -p 3.12 --env core     # check the core set
python scripts/verify-env.py -p 3.12 --all          # check every known set
python scripts/verify-env.py -p 3.12 --packages numpy pandas   # check your own list
```
...
```bash
./scripts/test-env.sh -p 3.12 01-core      # one environment  (Windows: .\scripts\test-env.ps1 -p 3.12 01-core)
./scripts/test-env.sh -p 3.12 --all        # all of them
```
...
two people creating `py312-core` a month apart might get slightly different versions. Most
...
|----------|-----------|----------------|-----------------|
| **A conda environment** (the actual installed Python + packages) | `create-env` / `conda env create` | Inside conda's own folder on your disk (e.g. `…/miniconda3/envs/py312-core`), **not** in this repo | No — it's on your machine only |
...
# 2) Build the daily-driver environment. (Linux/macOS shown; Windows: .\scripts\create-env.ps1 -p 3.12 01-core)
