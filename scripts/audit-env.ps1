...
.EXAMPLE
    .\audit-env.ps1 -p 3.12 04-web   # shorthand -> python\3.12\lockfiles\requirements\04-web.txt
#>
...
param(
    ...
    # -p/--python: required ONLY for the bare-name shorthand (to locate the tree).
...
switch ($kind) {
    ...
    'Req' {
        ...
        elseif ($Python) {
            ...
            if (Test-Path (Join-Path $reqDir "$Target.txt")) { $auditSrc = (Resolve-Path (Join-Path $reqDir "$Target.txt")).Path }
            else { Write-Error "no requirements file '$reqDir\$Target.txt'"; exit 2 }
        }
        else { Write-Error "shorthand '$Target' needs -p <X.Y> to locate python\<ver>\lockfiles\requirements\"; exit 2 }
