...
.DESCRIPTION
    ...
    This is a pure toolchain preflight, so -Python / -p is OPTIONAL: pass it to
    label the report with a Python-version context and version-specific next
    steps; omit it and the report is version-neutral.
...
$pytag     = if ($Python) { 'py' + ($Python -replace '\.', '') } else { '' }
$hintVer   = if ($Python) { $Python } else { '3.12' }
...
if ($Python) { Write-Host "conda-environments doctor - context: python/$Python ($pytag-*)" }
else { Write-Host 'conda-environments doctor - version-neutral (pass -p X.Y for a tree context)' }
Write-Host "==========================================================="
...
if ($solverOk) { Write-Host "Ready for the conda/mamba path. Next: .\scripts\create-env.ps1 -p $hintVer 01-core" }
else { Write-Host 'No conda-family solver found. Install Miniforge (conda+mamba) for the dev path.' }
if ((Test-Tool uv) -or (Test-Tool pip)) { Write-Host "Ready for the venv/uv path.   Next: .\scripts\setup-venv.ps1 -p $hintVer 04-web" }
