...
#   ./audit-env.sh -p 3.12 04-web                # shorthand -> python/3.12/lockfiles/requirements/04-web.txt
#
# Options:
#   -p, --python <X.Y>  Python tree for the bare-name shorthand (e.g. -p 3.12).
#                       Required ONLY for the shorthand form; the --name / --venv /
#                       --requirements targets do not need it.
...
# REQ_DIR is only meaningful for the bare-name shorthand, and only once we know
# the Python version (-p). It is resolved lazily below.
PYVER=""; REQ_DIR=""
...
Usage:
  ...
  $0 -p <X.Y> <name>               shorthand -> python/<X.Y>/lockfiles/requirements/<name>.txt
Options: -p/--python <X.Y>  --no-vulns  --json
EOF
  exit 2
}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    *)
      # bare arg -> a literal requirements path, or a shorthand needing -p
      if [[ -f "$1" ]]; then
        target_kind="req"; target="$1"
      elif [[ -n "$PYVER" ]]; then
        REQ_DIR="${REPO_ROOT}/python/${PYVER}/lockfiles/requirements"
        if [[ -f "${REQ_DIR}/$1.txt" ]]; then target_kind="req"; target="${REQ_DIR}/$1.txt"
        else echo "error: no requirements file '${REQ_DIR}/$1.txt'" >&2; usage; fi
      else
        echo "error: shorthand '$1' needs -p <X.Y> to locate python/<ver>/lockfiles/requirements/" >&2
        usage
      fi
      shift ;;
  esac
