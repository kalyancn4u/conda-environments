...
#
# Usage:
#   ./micromamba-env.sh -p 3.12 01-core      # env <pytag>-core from python/3.12/environments/01-core.yml, then verify
#   ./micromamba-env.sh -p 3.10 llm          # from python/3.10/templates/... (resolved via the tree)
#   MAMBA_ROOT_PREFIX=./mm ./micromamba-env.sh -p 3.12 01-core   # custom root prefix
#
...
VER_ROOT="${REPO_ROOT}/python/${PYVER}"           # python/<ver>
...
if [[ $# -lt 1 || -z "${1:-}" ]]; then
  echo "Usage: $0 -p <X.Y> <env-stem|path-to-yml>   e.g. $0 -p ${PYVER} 01-core" >&2
  exit 2
fi
...
if [[ -n "$verify_key" ]]; then
  ...
  "$MM" run --name "$env_name" python "${SCRIPT_DIR}/verify-env.py" -p "$PYVER" --env "$verify_key"
fi
