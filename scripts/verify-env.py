...
def check_python(min_minor: int) -> bool:
    """Warn (not fail) if the interpreter is not the expected 3.<min_minor> line."""
    major, minor = sys.version_info[:2]
    ok = (major, minor) == (3, min_minor)
    marker = "OK " if ok else "!! "
    print(f"{marker}python {platform.python_version()} "
          f"(expected 3.{min_minor}{'' if ok else ' - mismatch'})")
    return ok
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("-p", "--python", required=True, metavar="X.Y",
                        help="the Python version this environment targets, e.g. 3.12")
    ...
    try:
        expected_minor = int(args.python.split(".")[1])
    except (IndexError, ValueError):
        parser.error(f"--python must look like 3.12 (got {args.python!r})")

    check_python(expected_minor)
    print("-" * 60)
